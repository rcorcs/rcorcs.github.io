...
int main(){
	...
	#pragma omp parallel num_threads(4) //four threads will be executed in total
	{
		...
		//These commands here can also be executed concurrently with the following sections
		printf("Entry #1 is printed by thread: %d\n", omp_get_thread_num());

		#pragma omp sections
		{
			//This command is executed only once (by a single thread)
			printf("Entry #2 is printed by thread: %d\n", omp_get_thread_num());

			//The following sections are executed concurrently (in parallel)
			#pragma omp section
			{
